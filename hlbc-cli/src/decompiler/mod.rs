...
use crate::decompiler::ast::{
    add, decr, incr, not, sub, Constant, ConstructorCall, Expression, LoopScope, Scope, Scopes,
    Statement,
};
...
fn make_statements(code: &Bytecode, f: &Function) -> Vec<Statement> {
    ...
    for (i, o) in iter {
        ...
        match *o {
            ...
            Opcode::Label => scopes.push_scope(Scope::Loop(LoopScope::new())),
            Opcode::JAlways { offset } => {
                if offset < 0 {
                    if let Some(loop_) = scopes.pop_last_loop() {
                        statement = Some(Statement::While {
                            cond: Expression::Constant(Constant::Bool(true)),
                            stmts: loop_.stmts,
                        });
                    }
                }
        ...
        if let Some(stmt) = scopes.process(statement.take()) {
