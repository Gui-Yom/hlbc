...
use crate::decompiler::ast::{
    add, cst_bool, cst_float, cst_int, cst_null, cst_string, decr, eq, gt, gte, incr, lt, lte, not,
    noteq, sub, ConstructorCall, Expression, LoopScope, Scope, ScopeType, Scopes, Statement,
};
            ...
            if let Some((new, j)) = constructor_ctx {
                if new == $arg0 {
                    push_expr!(
                        j,
                        new,
                        Expression::Constructor(ConstructorCall::new(f.regtype(new), make_args!($($args),*)))
                    );
                }
            } else {
                push_expr!($i, $dst, Expression::Call($fun, make_args!($arg0 $(, $args)*)));
            }
        };
    }
        ...
        ($i:ident, $dst:expr, $fun:expr, $args:expr) => {
            if let Some((new, j)) = constructor_ctx {
                ...
                push_expr!(
                    $i,
                    $dst,
                    Expression::Call($fun, $args.iter().map(|x| expr!(x)).collect::<Vec<_>>())
                );
            }
        };
    }
